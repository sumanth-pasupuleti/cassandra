...
public abstract class AbstractCommitLogSegmentManager
{
     ...
     */
    void forceRecycleAll(Iterable<UUID> droppedTables)
    {
        ...
        try
        {
            ...
            for (CommitLogSegment segment : activeSegments)
                for (UUID tableId : droppedTables)
    ...
    private Future<?> flushDataFrom(List<CommitLogSegment> segments, boolean force)
    {
        ...
        {
            for (UUID dirtyTableId : segment.getDirtyTableIds())
            {
                CFMetaData metadata = Schema.instance.getCFMetaData(dirtyTableId);
                if (metadata == null)
                {
                ...
                {
                    final ColumnFamilyStore cfs = Keyspace.open(metadata.ksName).getColumnFamilyStore(dirtyTableId);
