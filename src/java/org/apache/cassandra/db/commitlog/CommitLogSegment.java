...
public abstract class CommitLogSegment
{
    ...
    private final NonBlockingHashMap<UUID, IntegerInterval> tableDirty = new NonBlockingHashMap<>(1024);

    // a map of Cf->clean intervals; separate map from above to permit marking Cfs clean whilst the log is still in use
    private final ConcurrentHashMap<UUID, IntegerInterval.Set> tableClean = new ConcurrentHashMap<>();
    ...
    static CommitLogSegment createSegment(CommitLog commitLog, AbstractCommitLogSegmentManager manager)
    {
        ...
        CommitLogSegment segment = config.useCompression() ? new CompressedSegment(commitLog, manager)
    ...
    Allocation allocate(Mutation mutation, int size)
    {
        ...
        try
        {
            ...
            for (PartitionUpdate update : mutation.getPartitionUpdates())
                coverInMap(tableDirty, update.metadata().cfId, position);
     ...
     */
    public synchronized void markClean(UUID tableId, CommitLogPosition startPosition, CommitLogPosition endPosition)
    {
    ...
    private void removeCleanFromDirty()
    {
        ...
        Iterator<Map.Entry<UUID, IntegerInterval.Set>> iter = tableClean.entrySet().iterator();
        while (iter.hasNext())
        {
            Map.Entry<UUID, IntegerInterval.Set> clean = iter.next();
            UUID tableId = clean.getKey();
     ...
     */
    public synchronized Collection<UUID> getDirtyTableIds()
    {
        if (tableClean.isEmpty() || tableDirty.isEmpty())
            return tableDirty.keySet();

        List<UUID> r = new ArrayList<>(tableDirty.size());
        for (Map.Entry<UUID, IntegerInterval> dirty : tableDirty.entrySet())
        {
            UUID tableId = dirty.getKey();
    ...
    public String dirtyString()
    {
        ...
        for (UUID tableId : getDirtyTableIds())
        {
            CFMetaData m = Schema.instance.getCFMetaData(tableId);
            sb.append(m == null ? "<deleted>" : m.cfName).append(" (").append(tableId)
