...
public abstract class CommitLogStressTest
{
    ...
    public CommitLogStressTest(ParameterizedClass commitLogCompression)
    {
    ...
    private void testLog(CommitLog commitLog) throws IOException, InterruptedException {
        ...
        if (hash == reader.hash && cells == reader.cells)
            System.out.format("Test success. compressor = %s, discarded = %d, skipped = %d\n",
                              commitLog.configuration.getCompressorName(),
                              reader.discarded, reader.skipped);
        else
        {
            System.out.format("Test failed (compressor = %s). Cells %d, expected %d, diff %d; discarded = %d, skipped = %d -  hash %d expected %d.\n",
                              commitLog.configuration.getCompressorName(),
