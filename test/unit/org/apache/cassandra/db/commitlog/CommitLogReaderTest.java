...
public class CommitLogReaderTest extends CQLTester
{
    ...
    private void confirmReadOrder(TestCLRHandler handler, int offset)
    {
        ...
        {
            PartitionUpdate pu = handler.seenMutations.get(i + j).getPartitionUpdate(currentTableMetadata().cfId);
            if (pu == null)
            {
                j++;
                continue;
    ...
    static class TestCLRHandler implements CommitLogReadHandler
    {
        ...
        public TestCLRHandler(CFMetaData metadata)
        {
        ...
        {
            if ((metadata == null) || (metadata != null && m.getPartitionUpdate(metadata.cfId) != null)) {
                seenMutations.add(m);
            }
        }
